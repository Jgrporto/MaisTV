...
export const registerChatArchitecture=async(app,{resolveSession,includeSse=true,includeBullBoard=true,customerSummaryProvider}={})=>{
  await initSentry();
  ...
  app.use('/api',await createChatRouter({authMiddleware:auth,customerSummaryProvider})); app.use('/api',await createAssignmentRouter({authMiddleware:auth})); app.use('/api',await createQueueConfigRouter({authMiddleware:auth})); app.use('/api',await createMediaRouter({authMiddleware:auth}));
