...
import { queueOutboundMediaMessage } from '../services/outbound-media.service.mjs';
...
export const createMediaRouter=async({authMiddleware}={})=>{
  ...
  router.post('/media/send',express.json({limit:'72mb'}),asyncRoute(async(req,res)=>res.status(202).json({item:await queueOutboundMediaMessage({auth:req.chatAuth,input:req.body||{}})})));
